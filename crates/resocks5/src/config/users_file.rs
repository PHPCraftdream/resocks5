...
/// Shared Windows-only test helper: asserts that `path` (an existing file)
/// carries exactly the protected owner-only DACL that
/// `platform::create_private_tmp` is specified to produce — a protected
/// DACL with a single allow ACE granting FILE_ALL_ACCESS to the
/// Owner-Rights SID (S-1-3-4) and nothing to anyone else. Extracted so the
/// users-file tests and the load_or_init migration tests assert the same
/// FFI-checked invariant without duplicating the raw calls.
